setwd("E:/PhD 1st year/2-3 PaPer2/R")
...
#data
data <- read.dta13("E:/PhD 1st year/2-3 PaPer2/R/paper02_merge_0927.dta")
newdata <- subset(data, data$RO5_2>4 & data$RO5_2<18)
...
#samppling
sample <- data2 %>% group_by(AGE)
sample2 <- sample_frac(sample, 0.05)


#graph

g1 <- ggplot(data2, aes(x=AGE, y=workinghours, fill=SEX)) +
...
#jags parallel
jags.fit1 <- jags.parallel(data=sample2, inits=NULL, params, model.file=model1,  n.chains = 2,  n.thin=5,  n.iter=10000, n.burnin = 1000, DIC=TRUE, jags.module = c("glm", "dic"))
